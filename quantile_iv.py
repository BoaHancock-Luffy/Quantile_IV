...
from ml_mr.logging import info
from ml_mr.utils import (
    default_validate_args,
    parse_project_and_run_name,
    initialize_meta
)
from ml_mr.utils.models import MLP, OutcomeMLPBase
from ml_mr.utils.quantiles import QuantileLossMulti
from ml_mr.utils.training import train_model, resample_dataset
from ml_mr.utils.data import IVDataset, IVDatasetWithGenotypes
from ml_mr.utils import _cat
from ml_mr.estimation.core import MREstimator
...
########################################################## 连续outcome ############################################################
   ...
   ################################################################################################################################
    ...
    #xmj添加：使结果能计算概率 然后转化成OR值
    # def forward(self, ivs: torch.Tensor, covars: Optional[torch.Tensor] = None, taus: Optional[torch.Tensor] = None):
    #     x_hats = self.exposure_network(_cat(ivs, covars))
    #     n_q = x_hats.size(1)
    #     n = ivs.size(0)
    
    #     y_hat = torch.zeros((n, 1), device=self.device)
    #     for j in range(n_q):
    #         y_hat += self.mlp(_cat(x_hats[:, [j]], covars)) / n_q
    
    #     if self.hparams.binary_outcome:  # type: ignore
    #         y_hat = torch.sigmoid(y_hat)  # <--- 输出概率
    #     return y_hat
